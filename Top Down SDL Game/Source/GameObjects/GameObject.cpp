...
void GameObject::RenderDebug(SDL_Renderer* renderer, Camera camera)
{
	if (m_render_initialized && !m_deletion && m_collider.GetActive())
	{
		...
		Transform t = camera.ConvertedToScreenSpace({ m_transform.position + offset, m_transform.scale * m_collider_scale * m_debug_box_scale });
		SDL_Rect des = { t.position.x, t.position.y, t.scale.x, t.scale.y };
		SDL_SetRenderDrawColor(renderer, 255, 255, 0, 255);
		SDL_RenderDrawRect(renderer, &des);

		m_collider_debug.RenderUpdate(
			renderer,
			camera.ConvertedToScreenSpace({ m_transform.position + offset, m_transform.scale * m_collider_scale * m_debug_box_scale }));
